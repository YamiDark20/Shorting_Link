...
import { deleteSelectedLink } from "../../redux/links_user_slice";

export const LinkDeletePage = ({toast, t, setIsToastVisible, info_link}) => {
    ...
    return (
        <div className='main_link_delete'>
            <p className='mensaje_delete'>¿Estas seguro de que quieres eliminar el link seleccionado?</p>
            <div className='div_botones_delete'>
                <button className='boton_confirmar_delete' onClick={async () => {
                    dispatch(deleteSelectedLink(info_link)).unwrap()
                    .then(() => {
                        setIsToastVisible(false);
                        toast.dismiss(t.id)
                        toast.success("Se ha eliminado correctamente el link del usuario", {
                            position: 'bottom-right',
                            style: getErrorToastStyles(window.innerWidth)
                        })
                    })
